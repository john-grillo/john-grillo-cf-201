...
//This block will check to see if there are any copies of a game in progress that isn't completed.
//Will also remove gamelog/questionNumber [qNumber]. qNumber will make sure you are in the right spot.
//This is really a working copy and a backup copy so that one rewrites the other. scoreTracker is the backup for my answerCount.
// Changing answerCount to something else will break things. don't know why.
//
//Check to see if NULL. If not NULL, get the score and resume game.
  ...
  if(checkAnswers == NULL) {
    ...
    answerCount = localStorage.getItem('scoreTracker');
  }
...
if(localStorage.getItem('qNumber') == 1) {
  var answerCount = 0;
   localStorage.removeItem('scoreTracker');
   localStorage.setItem('scoreTracker', answerCount);
